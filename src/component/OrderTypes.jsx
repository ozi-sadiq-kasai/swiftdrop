import { icons, places, foodItems } from '../data'
...
const OrderTypes = () => {
  ...
  const filteredPlaces = places.filter((place) => place.iid === selectedIid)
  // const filteredFoodItems = foodItems.filter((item) => item.sid === selectedIid)
  ...
  return (
    <div className="w-screen h-dvh">
      ...
      <div className="flex justify-around items-center flex-wrap xsm:justify-evenly xsm:gap-2 my-8">
        {icons.map((icon) => (
          <div key={icon.iid} onClick={() => handleIconClick(icon.iid)}>
            <img
              src={icon.icon}
              alt={icon.name}
              className="w-16 cursor-pointer xsm:w-10"
            />
      ...
      <OrderPlaces places={filteredPlaces} />
